...
## Мульти-стрим (issue #50)

Распределяет один туннель по **N документам Yandex**, чтобы при падении WebSocket одного документа туннель продолжал работать через оставшиеся N-1. Активируется передачей нескольких URL через запятую в `--url`:
...
# клиент (тот же набор URL)
./universal-bypass-tool --client \
    --url "https://disk.yandex.ru/i/AAA,https://disk.yandex.ru/i/BBB,https://disk.yandex.ru/i/CCC" \
    --socks5 :1080 --debug
```

Один URL сохраняет стандартное поведение одного канала.

**Как это работает:**
- **Send** выбирает следующий живой стрим по round-robin; мёртвые пропускаются автоматически.
- **Receive** собирает данные из всех стримов; порядок корректен, так как туннель несёт TCP.
- **IsConnected** — OR по всем стримам: туннель работает, пока жив хотя бы один документ.

**Наблюдаемость:** добавьте `--multistream-status 5s` для периодического лога состояния стримов:
```
[MULTI] up=2/3 s0=UP(rx=122,tx=106,rc=0) s1=DOWN(rx=0,tx=0,rc=3) s2=UP(rx=120,tx=104,rc=1)
```

**Мобильные клиенты:** мульти-стрим работает на iOS и Android. На iOS передайте URL через запятую в поле URL. На Android CLI-бинарник поддерживает `--url url1,url2,...` нативно.

**Примечания:**
- Оба пира (клиент и выходная нода) должны использовать одинаковый набор URL.
- Работает с транспортами `yandex` и `vyandex`.
- `cupsonline` уже несёт multi-room состояние внутри своего base64-URL и не поддерживает мульти-URL через запятую.
...
│   ├── multistream.go          # Мульти-стрим обёртка (N транспортов как один)
...
| `--url`       | `https://localhost` | URL документа(ов). Через запятую для мульти-стрим |
| `--multistream-status` | `0` | Лог состояния стримов с заданным интервалом (напр. `5s`). Не работает с одним URL |
