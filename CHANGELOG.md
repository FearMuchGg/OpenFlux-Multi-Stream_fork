...
- [Unreleased — Batching + OFSP](#-batching--ofsp-unreleased)
- [OFSP v1 — 2026-09-14](#-ofsp-v1-2026-09-14)
...
**Производительное обновление**: добавлен слой батчинга поверх OFSP, который уменьшает количество WebSocket-сообщений в **6-62 раза** и увеличивает пропускную способность до **6 MB/s**.

### 📋 Проблема

Каждое WebSocket-сообщение к Yandex.Docs несло один IP-пакет с огромными накладными расходами:
- JSON-обёртка Socket.IO
- Base64 кодирование
- TCP/IP заголовки

На ACK-трафике (мелкие пакеты) это давало тучу сообщений и фиксированные накладные расходы на каждое.

### ⚡ Решение

**BatchedTransport** — новый слой поверх OFSP, который:
1. **Копит пакеты** в очереди (`queue chan []byte`)
...
### 📦 Формат кадра (wire format)

```
[1 байт: version = 0x02]
[1 байт: flags]
  └─ bit 0: compressed (zstd applied)
[пакеты]:
  [2 байта: длина пакета] + [данные пакета]
  [2 байта: длина пакета] + [данные пакета]
  ...
```
...
```bash
OPENFLUX_BATCH_BYTES=8192      # макс. размер пачки в байтах
OPENFLUX_BATCH_COUNT=64        # макс. количество пакетов в пачке
OPENFLUX_BATCH_LINGER_MS=5     # сколько ждать "stragglers" после burst
```

### 🔄 Алгоритм flush

**Фаза 1: Burst coalescing**
- Забираем всё, что уже в очереди (без ожидания)
- Склеиваем до `maxBatchCount` пакетов или `maxBatchBytes` байт

**Фаза 2: Linger (опционально)**
- Ждём 5ms, вдруг придут ещё пакеты
- Добавляем их в батч
- Отправляем после таймаута или при достижении лимитов
...
Новый формат кадра **несовместим** со старым LZ4-форматом:
- **Новый → старый**: ❌ не работает (старый увидит 0x02 в начале и отбросит)
- **Старый → новый**: ❌ не работает (новый не распознает старый формат)

**Обновлять нужно ОБА конца одновременно** (клиент и exit-node).

**Использование флага `--legacy`**:
```bash
# Legacy режим (без батчинга, только OFSP)
...
Send(IP packet)
  ↓
OFSP: wrap → [0xFF][flags][seq][payload]  (11 байт заголовок)
  ↓
Batched: coalesce → [0x02][flags][len+OFSP1][len+OFSP2]...  (несколько OFSP в одном кадре)
  ↓
Yandex: base64 + Socket.IO → WebSocket
```

**На приёмнике:**
1. Yandex получает кадр → base64 decode
2. Batched распаковывает батч → список OFSP-конвертов
3. OFSP обрабатывает каждый конверт отдельно (seq, ACK, дедупликация)

### 🎯 Критичные пакеты (SYN/FIN/RST)

Критичные TCP-пакеты отправляются **в обход батчинга** для минимальной задержки:
- OFSP помечает их флагом `flagCritical`
- BatchedTransport проверяет этот флаг
- Отправляет критичные пакеты напрямую через `SendImmediate()` без задержки в 5ms

Это гарантирует, что установка TCP-соединения не замедляется батчингом.
